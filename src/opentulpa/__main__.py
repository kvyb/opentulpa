...
def _load_or_create_secret_cipher(data_root: Path) -> AesGcmHostKeyCipher:
    """Load the root-of-trust key from env or a host-only 0600 file."""

    configured = str(os.environ.get("OPENTULPA_SECRET_VAULT_KEY", "") or "").strip()
    if configured:
        return AesGcmHostKeyCipher.from_base64(configured)

    key_path = data_root / "bootstrap" / "secret-vault.key"
...
def build_application(*, project_root: Path, settings: Settings) -> ApplicationComposition:
    ...
    try:
        ...
        secret_vault_store = SecretVault(
            ...
            cipher=_load_or_create_secret_cipher(data_root),
        )
