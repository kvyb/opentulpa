...
PROMPT_POLICY_BLOCKS: list[tuple[str, str, list[tuple[str, str]]]] = [
    (
        ...
        [
            ...
            ("A20", "In interactive chat only, for complex multi-step work, call turn_plan when a private current-turn checklist would help: create a realistic current-turn plan with a clear goal that can fit the available runtime, keep exactly one step in_progress, update statuses as work moves forward, and answer with the best complete or partial result before reply timeout. Do not use turn_plan for simple chat, routine wakes, workflow setup, or inbound/customer-message execution."),
        ],
    ),
    (
        "B",
