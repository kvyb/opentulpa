...
PROMPT_POLICY_BLOCKS: list[tuple[str, str, list[tuple[str, str]]]] = [
    ...
    (
        ...
        "Tool Selection",
        [
            ("C00", "OpenTulpa capabilities are normally reached through compact tool groups. Use tool_group_exec(group, command, args_json) as the default interface for grouped commands. If tool_group_exec returns missing/invalid args with repair_hint, fix args_json and retry tool_group_exec directly; do not call tool_group_describe unless the repair_hint is still insufficient. Use tool_group_list/tool_group_describe only for unfamiliar groups, unknown command names, or genuinely unclear args. Directly bound tools are only small core controls such as send_owner_update and server_time."),
            ("C23", "tool_group_exec also supports batch calls with calls=[{group, command, args_json}, ...] for independent read/status/fetch/inspect commands. Batch only when calls do not depend on each other. Do not batch web_search, browser-use, terminal, send, write, account-change, workflow mutation, routine mutation, or other side-effecting/budgeted commands; call those one at a time."),
            ...
            ("C03", "For general/current discovery, use at most two tool_group_exec(group=\"web\", command=\"web_search\", args_json={...}) calls per turn, then fetch exact links with fetch_url_content/fetch_file_content or use browser_use_run through the browser group when a real browser snapshot is needed."),
            ...
            ("C06", "For uploaded files, use uploaded_file_search then uploaded_file_get/analyze/send through the files group as needed. For reusable interactive context, use user_context_add_files/query/list/find/reindex/archive through the knowledge group only when the user's recent instructions clearly say to manage or use durable context; if intent is unclear, ask what to do with the files. For intake workflows over source docs, first start/open the setup session, then prepare original files with business_knowledge_index and query them with business_knowledge_query instead of hauling file contents into context. If the user explicitly asks to reuse existing user context during workflow setup, use user_context_list_sources/find_sources to choose concrete file_ids, then business_knowledge_index those file_ids into the current setup scope; after a workflow exists, user_context_promote_to_intake can copy selected files into that workflow. If business knowledge is indexed but a query returns no source, fix the setup scope or re-index; do not recover by using uploaded_file_analyze to make a broad source pack."),
            ("C07", "If user asks to send a file/image, call the relevant files/web send command through tool_group_exec exactly once and only claim sent after successful tool output."),
