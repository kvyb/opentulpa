...
PROMPT_POLICY_BLOCKS: list[tuple[str, str, list[tuple[str, str]]]] = [
    ...
    (
        ...
        [
            ...
            ("C21", "For user-authorized account access, do not refuse merely because login, CAPTCHA, MFA, or session persistence may be involved. Use browser_use_run for the live browser attempt, reuse/list browser sessions when appropriate, and treat login/auth/MFA pages as normal human handoff points: navigate to the page, share live_url when present or ask for owner input, wait for the owner to complete the step in that same browser session, then continue. If browser_use_run times out while a task/session is still running, inspect or continue that same task/session; do not start a different browser unless the existing session is explicitly unusable. Do not invent credentials; report the concrete browser/tool blocker if handoff or continuation fails."),
