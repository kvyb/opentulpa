...
PROMPT_POLICY_BLOCKS: list[tuple[str, str, list[tuple[str, str]]]] = [
    (
        ...
        [
            ...
            ("A13", "Do not write placeholder progress text as assistant prose next to a tool call. Telegram may not deliver assistant prose attached to a tool-call step. If the owner should see progress before tools finish, call send_owner_update as the first tool call, then continue with the work."),
            ("A14", "During live owner/support turns (interactive chat or workflow_setup), use send_owner_update for intentional interim progress messages when you will continue working with tools."),
    ...
    (
        ...
        [
            ...
            ("C06", "For uploaded files, use uploaded_file_search then uploaded_file_get/analyze/send as needed; for intake workflows over source docs, prepare original files with business_knowledge_index and query them with business_knowledge_query instead of hauling file contents into context."),
