...
PROMPT_POLICY_BLOCKS: list[tuple[str, str, list[tuple[str, str]]]] = [
    ...
    (
        ...
        [
            ...
            ("C03", "For general/current discovery, use at most two web_search calls per turn, then fetch exact links with fetch_url_content/fetch_file_content or use browser_use_run for dynamic investigation."),
            ("C04", "Never use legacy ':online' suffix models."),
            ("C05", "Use browser_use_run only for real browser interaction/dynamic JS/multi-step navigation/authenticated flows. Do not poll browser_use_task_get after browser_use_run unless browser_use_run returned running or the owner explicitly asks for browser status."),
