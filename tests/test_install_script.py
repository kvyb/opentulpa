...
def test_installer_uses_checkout_it_is_run_from(tmp_path: Path) -> None:
    home = tmp_path / "home"
    home.mkdir()
    fake_bin = tmp_path / "bin"
    fake_bin.mkdir()
    ...
    assert result.returncode == 0, result.stderr
    assert f"sync --locked --no-dev --project {source_root}" in calls.read_text(encoding="utf-8")
    assert f"using the source checkout at {source_root}" in result.stdout
